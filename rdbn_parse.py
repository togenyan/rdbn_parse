    ...
    for l in sorted(lists.values(), key=list_sorter):
        ...
        for i in range(l.count):
            ...
            for col, conv in zip(table_type.columns, convertors):
                if last_offset:
                    ...
                    if pad > 0:
                        pos += pad
                        if i == 0:
                            logger.debug("{}-byte padding inserted at {}".format(
                                pad, pos
                            ))
                    elif pad < 0:
            ...
            if i == 0 and pos != l.size:
                logger.debug("data reading ends at {}, leaving {} byte unread".format(pos, l.size - pos))
                logger.debug("unread data (only the first row will be shown): {}".format(row_data[pos:]))
